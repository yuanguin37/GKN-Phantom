...
def _fingerprint_first(body: str) -> tuple[str, str] | None:
    """First (engine, signature) matching ``body`` via an AC-prefiltered
    regex set — identical result to scanning every signature in order.

    v5.5: one Aho-Corasick pass over the body selects which signature
    regexes can match before any of them runs (pattern_matcher.py).
    """
    if not hasattr(_fingerprint_first, "_set"):
        _fingerprint_first._set = PrefilteredRegexSet(  # type: ignore[attr-defined]
            list(enumerate(sig for _engine, sig in _fingerprint_signatures())),
            flags=re.IGNORECASE,
        )
    for idx, _m in _fingerprint_first._set.search_all(body):  # type: ignore[attr-defined]
        return _fingerprint_signatures()[idx]
    return None
