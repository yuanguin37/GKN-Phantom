...
# v5.5 — one AC-gated set over the three HTML body tables, registered in
# table order (meta → cms → script) so _detect_tech_from_html's match order
# and cross-table `seen` dedup are unchanged. See pattern_matcher.py.
_TECH_HTML_SET = PrefilteredRegexSet(
    [(("meta", i), e[1]) for i, e in enumerate(META_GENERATOR_PATTERNS)]
    + [(("cms", i), e[1]) for i, e in enumerate(CMS_DIRECT_PATTERNS)]
    + [(("script", i), e[1]) for i, e in enumerate(SCRIPT_FRAMEWORKS)]
)
...
def _detect_tech_from_html(html: str) -> list[dict[str, Any]]:
    ...
    # v5.5: the three body tables run through one AC-gated pass
    # (pattern_matcher.PrefilteredRegexSet); entries are registered in
    # table order so match order — and therefore the cross-table `seen`
    # dedup — is identical to iterating each table in turn.
    for key, m in _TECH_HTML_SET.search_all(html):
        kind, i = key
        if kind == "meta":
            name, _pattern = META_GENERATOR_PATTERNS[i]
            category = "cms"
            version = m.group(1) if m.lastindex and m.lastindex >= 1 else None
        elif kind == "cms":
            name, _pattern = CMS_DIRECT_PATTERNS[i]
            category = "cms"
            version = None
        else:  # "script"
            name, _pattern = SCRIPT_FRAMEWORKS[i]
            category = "javascript"
            version = m.group(1) if m.lastindex and m.lastindex >= 1 else None
        if name in seen:
            continue
